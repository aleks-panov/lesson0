...
async def post_user(username: Annotated[str, Path(ge=5, le=20, description="Enter username", example="Urban")],
...
async def update_user(user_id: Annotated[int, Path(ge=1, description="Inter user ID", example="1")],
                      username: Annotated[str, Path(ge=5, le=20, description="Enter username", example="Urban")],
                      age: Annotated[int, Path(ge=18, le=120, description="Enter age", example="18")]) -> str:
    message = f"Имя: {username}, возраст: {age}"
    users[user_id: str] = message
